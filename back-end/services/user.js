...
export const onSocialLogin = async (payload) => {
    const userData = await User.findOneByCondition({ $or:[{[`social.${payload.social.key}`]: payload.social.value },{email: payload["email"] || "-"}]});
    ...
    payload.username = `${payload.firstName.toLowerCase()}${generateRandom(4, false).toLowerCase()}`;
    ...
    if(!userData){
        ...
        const coordinates = updateObject["location"] ? updateObject.location.coordinates : [];
        ...
        data = await User.onLoginDone(userData._id, {...payload, token}, getLoginType(payload.social.key));
    }  
    return {
        _id: data._id,
        firstName: data.firstName,
...
export const usernames = async (payload) => {
    if(await User.findOneByCondition(payload)) throw new Error(Message.userNameAlreadyExists);
    return [...new Array(4)].map(() => `${payload.username.toLowerCase()}${generateRandom(4, false).toLowerCase()}`); 
}
...
export const OTP = async (payload) => {
    if(payload.type === 'send_otp'){
        ...
        return await User.updateUser(payload.userId, {"phone.number": payload.number, "phone.otp": otp});
    } else {
        if(!await User.findOneByCondition({_id: payload.userId, 'phone.otp': payload.number})) throw new Error(Message.invalidOtp);
        return await User.updateUser(payload.userId, {"phone.otp": "", "phone.verification": true});
    }    
}